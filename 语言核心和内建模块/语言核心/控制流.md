...
* `break` 语句仅允许在`循环体内使用`。`执行 break` 后，`循环终止`。如果一个循环嵌套在其他循环内，则 `break` 仅`终止`最`内层`的`嵌套循环`。在实践中，`break` 通常位于循环体内 `if 的子句`中，因此可以`有条件地执行 break`。
...
Sometimes, a continue statement can take the place of nested if statements within a loop.
